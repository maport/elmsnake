var Elm = Elm || { Native: {} };
Elm.AirConsole = Elm.AirConsole || {};
Elm.AirConsole.make = function (_elm) {
   "use strict";
   _elm.AirConsole = _elm.AirConsole || {};
   if (_elm.AirConsole.values)
   return _elm.AirConsole.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "AirConsole",
   ...
   var usersFromMessage = A2($Json$Decode._op[":="],
   "user_ids",
   $Json$Decode.list($Json$Decode.$int));
   var parseUsersFromMessage = function (msg) {
      return $Result.map($Set.fromList)(A2($Json$Decode.decodeValue,
      usersFromMessage,
      msg));
   };
   var UserUpdate = function (a) {
      return {ctor: "UserUpdate"
             ,_0: a};
   };
   ...
   var DeviceChange = F2(function (a,
   b) {
      return {ctor: "DeviceChange"
             ,_0: a
             ,_1: b};
   });
   ...
   var deviceState = A5($Json$Decode.object4,
   DeviceState,
   A2($Json$Decode._op[":="],
   "uid",
   $Json$Decode.string),
   ...
   var message = A2($Json$Decode.andThen,
   A2($Json$Decode._op[":="],
   "msg",
   $Json$Decode.string),
   body);
   var parseMessage = $Json$Decode.decodeValue(message);
   var actionsFromMessage = function (msg) {
      return function () {
         var is_ok = function (r) {
            return function () {
               switch (r.ctor)
               {case "Ok": return true;}
               return false;
         ...
         var results = _L.fromArray([$Result.map(UserUpdate)(parseUsersFromMessage(msg))
                                    ,parseMessage(msg)]);
         return $List.map(function ($) {
            return $Maybe.withDefault(Ready("?"))($Result.toMaybe($));
         })(A2($List.filter,
         is_ok,
         results));
      }();
   };
   var mailbox = $Signal.mailbox({ctor: "_Tuple2"
                                 ,_0: $Maybe.Nothing
                                 ,_1: $Json$Encode.$null});
   var outSignal = mailbox.signal;
   var sendMessage = F2(function (userId,
   data) {
   ...
   _elm.AirConsole.values = {_op: _op
                            ,sendMessage: sendMessage
                            ,actionsFromMessage: actionsFromMessage
                            ,outSignal: outSignal
                            ,Ready: Ready
...
Elm.App.make = function (_elm) {
   ...
   _L = _N.List.make(_elm),
   $moduleName = "App",
   $AirConsole = Elm.AirConsole.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   var airConsoleOut = Elm.Native.Port.make(_elm).outboundSignal("airConsoleOut",
   function (v) {
      return [v._0.ctor === "Nothing" ? null : v._0._0
             ,v._1];
   },
   $AirConsole.outSignal);
   var airConsoleIn = Elm.Native.Port.make(_elm).inboundSignal("airConsoleIn",
   "AirConsole.Message",
   function (v) {
      return v;
   });
   var userDataDecoder = function () {
      var toPos = function (dir) {
         return function () {
            switch (dir)
            {case "down": return {_: {}
                                 ,x: 0
   ...
   var maybeSpawnFood = function (model) {
      return function () {
         var shouldSpawn = function () {
            var _v1 = {ctor: "_Tuple3"
                      ,_0: _U.cmp($List.length(model.foods),
                      5) < 0
                      ,_1: model.clockTime
                      ,_2: model.lastFoodSpawn};
            switch (_v1.ctor)
            {case "_Tuple3": switch (_v1._0)
                 {case true: switch (_v1._1.ctor)
                      {case "Just":
                         switch (_v1._2.ctor)
                           {case "Just":
                              return _U.cmp($Time.inSeconds(_v1._1._0 - _v1._2._0),
                                10) > 0;
         ...
         return shouldSpawn ? _U.replace([["foods"
                                          ,A2($List._op["::"],
                                          $Model.makeFood(model),
                                          model.foods)]
                                         ,["lastFoodSpawn"
                                          ,model.clockTime]],
         model) : model;
   ...
   var eatFoods = F3(function (maxLen,
   players,
   foods) {
      return function () {
         var eatFood = function (player) {
            return $Model.updateScore(5)(_U.replace([["length"
                                                     ,A2($Basics.min,
                                                     player.length + 1,
                                                     maxLen)]],
            player));
         };
         var checkFoods = F3(function (id,
         player,
         _v7) {
            return function () {
               switch (_v7.ctor)
                  ...
                  return function () {
                       ...
                       {case "Just":
                          return {ctor: "_Tuple2"
                                 ...
                                 ,_1: A2($List.filter,
                                 function ($) {
                                    return $Basics.not(F2(function (x,
                                    y) {
                                       return _U.eq(x,y);
                                    })(_v11._0)($));
                                 },
         ...
         A2($Dict.filter,
         function (id) {
            return $Model.playerIsLive;
         },
         players));
      }();
   });
   var movePlayers = F2(function (clockTime,
   model) {
      return function () {
         var _v13 = model.clockTime;
         switch (_v13.ctor)
         {case "Just":
            return function () {
                 var delta = $Time.inSeconds(clockTime - _v13._0);
                 var dist = 4 * delta;
                 var movedPlayers = A2($Dict.map,
                 function (id) {
                    return $Model.movePlayer(dist);
                 },
                 model.players);
                 var livePlayers = $List.filter($Model.playerIsLive)($Dict.values(movedPlayers));
                 var checkPlayers = F3(function (id,
                 p,
                 players) {
                    return function () {
                       var wall = A2($Model.hitEntity,
                       model.walls,
                       p);
                       var otherPlayer = A2($Model.hitEntity,
                       livePlayers,
                       p);
                       return function () {
                          ...
                          {case "_Tuple2":
                             switch (_v15._0.ctor)
                               {case "Just":
                                  return function () {
                                       var players$ = A3($Dict.update,
                                       p.id,
                                       $Maybe.map($Model.killPlayer(model)),
                                       players);
                                       return $Model.playerIsLive(p) ? A3($Dict.update,
                                       _v15._0._0.id,
                                       $Maybe.map($Model.updateScore(10)),
                                       players$) : players$;
                                    }();}
                               switch (_v15._1.ctor)
                               {case "Just":
         ...
         _U.badCase($moduleName,
         "between lines 48 and 79");
      }();
   });
   var ConsoleMsgSent = function (a) {
      return {ctor: "ConsoleMsgSent"
             ,_0: a};
   };
   var notifyNewPlayers = function (players) {
      return function () {
         var sendMsg = function (p) {
            return $Task.map(ConsoleMsgSent)($Task.toMaybe(A2($AirConsole.sendMessage,
            $Maybe.Just(p.id),
            $Json$Encode.string($View.playerColour(p.id)))));
         };
         var delayedSend = function (p) {
            return A2($Utils.delayedTask,
            5 * $Time.second,
            sendMsg(p));
         };
         return $Effects.batch(A2($List.map,
         function ($) {
            return $Effects.task(delayedSend($));
         },
         players));
      }();
   };
   var handleConsoleAction = F2(function (action,
   _v20) {
      return function () {
         switch (_v20.ctor)
         {case "_Tuple2":
                 ...
                 {case "Data":
                    return function () {
                         ...
                         return function () {
                            ...
                            {case "Just":
                               return function () {
                                    var dir$ = $Maybe.withDefault(player._0.dir)($Result.toMaybe(A2($Json$Decode.decodeValue,
                                    userDataDecoder,
                                    action._1)));
                                    var player$ = $Model.makePlayerLive(A2($Model.changePlayerPendingDir,
                                    dir$,
                                    player._0));
                                    return {ctor: "_Tuple2"
                    ...
                    return function () {
                         ...
                         staying = $._0,
                         leaving = $._1;
                         var arriving = $Dict.fromList($List.map(function (id) {
                            return {ctor: "_Tuple2"
                                   ,_0: id
                                   ,_1: A2($Model.makePlayer,
                                   _v20._0,
                                   id)};
                         })($List.take(_v20._0.maxPlayers - $List.length($Dict.keys(staying)))($Set.toList(A2($Set.filter,
                         function (k) {
                         ...
                         action._0)))));
                         var notifyEffects = notifyNewPlayers($Dict.values(arriving));
                         var players$ = A2($Dict.union,
                         staying,
                         arriving);
                         return {ctor: "_Tuple2"
                                ,_0: _U.replace([["players"
                                                 ,players$]],
                                _v20._0)
                                ,_1: $Effects.batch(_L.fromArray([_v20._1
                                                                 ,notifyEffects]))};
                      }();}
                 return {ctor: "_Tuple2"
                        ,_0: _v20._0
   ...
   var init = {ctor: "_Tuple2"
              ,_0: {_: {}
                   ,clockTime: $Maybe.Nothing
                   ,foods: _L.fromArray([])
                   ,halfSize: {_: {},x: 20,y: 20}
                   ,lastFoodSpawn: $Maybe.Nothing
                   ,maxPlayers: 5
                   ,maxSnakeLength: 50
                   ,players: $Dict.empty
                   ,walls: _L.fromArray([{_: {}
                                         ,line: _L.fromArray([{_: {}
                                                              ,x: -20
                                                              ,y: -20}
                                                             ,{_: {}
      ...
      return function () {
         ...
         {case "ConsoleMsg":
            return A3($List.foldl,
              ...
              ,_0: model
              ,_1: $Effects.none},
              $AirConsole.actionsFromMessage(action._0));
            case "Tick":
            return function () {
                 var players$ = A2(movePlayers,
                 action._0,
                 model);
                 var $ = A3(eatFoods,
                 model.maxSnakeLength,
                 players$,
                 model.foods),
                 players$$ = $._0,
                 foods$ = $._1;
                 return {ctor: "_Tuple2"
                        ,_0: maybeSpawnFood(_U.replace([["players"
                                                        ,players$$]
                                                       ,["foods",foods$]
                                                       ,["clockTime"
   ...
   var ConsoleMsg = function (a) {
      return {ctor: "ConsoleMsg"
             ,_0: a};
   };
   ...
   _elm.App.values = {_op: _op
                     ...
                     ,Tick: Tick
                     ,ConsoleMsgSent: ConsoleMsgSent
                     ,init: init
                     ,movePlayers: movePlayers
                     ,eatFoods: eatFoods
                     ,maybeSpawnFood: maybeSpawnFood
                     ,userDataDecoder: userDataDecoder
                     ,notifyNewPlayers: notifyNewPlayers
                     ,handleConsoleAction: handleConsoleAction
                     ,update: update
                     ,app: app
                     ,main: main};
   return _elm.App.values;
...
Elm.Model.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm),
   $Utils = Elm.Utils.make(_elm);
   var nextTurnPosition = F2(function (dir,
   pos) {
      return function () {
         var $ = _U.eq(dir.x,
         0) ? {ctor: "_Tuple2"
              ...
              ,_1: function (f) {
                 ...
                 pos);
                   ...
                   ,_1: function (f) {
                      return _U.replace([["x"
                                         ,$Basics.toFloat(f(pos.x))]],
                      pos);
                   }},
         movingAxis = $._0,
         updatePos = $._1;
         return updatePos(_U.cmp(movingAxis(dir),
         0) > 0 ? $Basics.ceiling : $Basics.floor);
      }();
   });
   var movePosition = F3(function (dir,
   dist,
   pos) {
      return {_: {}
             ,x: pos.x + dist * dir.x
   ...
   var collisionPoint = function (player) {
      return $Maybe.map(A2(movePosition,
      player.dir,
      0.5))($List.head(player.line));
   };
   var isSegmentHit = F2(function (pos,
   _v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return function () {
                 var _v4 = {ctor: "_Tuple2"
                           ,_0: _U.eq(_v0._0.x,_v0._1.x)
                           ,_1: _U.eq(_v0._0.y,_v0._1.y)};
                 switch (_v4.ctor)
                      ...
                      {case true: switch (_v4._1)
                           {case false:
                              return _U.cmp(pos.y,
                                A2($Basics.min,
                                _v0._0.y,
                                _v0._1.y)) > -1 && (_U.cmp(pos.y,
                                A2($Basics.max,
                                _v0._0.y,
                                _v0._1.y)) < 1 && _U.cmp($Basics.abs(pos.x - _v0._0.x),
                                0.5) < 0);
                              case true:
                              return _U.cmp($Basics.abs(pos.x - _v0._0.x),
                                0.5) < 0 && _U.cmp($Basics.abs(pos.y - _v0._0.y),
                                0.5) < 0;}
                           break;}
                      break;}
                 return _U.cmp(pos.x,
                 A2($Basics.min,
                 _v0._0.x,
                 _v0._1.x)) > -1 && (_U.cmp(pos.x,
                 A2($Basics.max,
                 _v0._0.x,
                 _v0._1.x)) < 1 && _U.cmp($Basics.abs(pos.y - _v0._0.y),
                 0.5) < 0);
              }();}
         _U.badCase($moduleName,
   ...
   var lineSegments = function (line) {
      return function () {
         switch (line.ctor)
         {case "::":
            return A3($List.map2,
   ...
   var entityAtPos = F2(function (entities,
   pos) {
      return function () {
         var collidesWith = function (e) {
            return $List.any(isSegmentHit(pos))(lineSegments(e.line));
         };
   ...
   var hitEntity = F2(function (entities,
   player) {
      return function () {
         var playerPos = collisionPoint(player);
         return function () {
            switch (playerPos.ctor)
            {case "Just":
               return A2(entityAtPos,
                 entities,
                 playerPos._0);
               case "Nothing":
               return $Maybe.Nothing;}
   ...
   var segmentDir = F2(function (pos1,
   pos2) {
      return {_: {}
             ,x: $Utils.sign(pos2.x - pos1.x)
             ,y: $Utils.sign(pos2.y - pos1.y)};
   });
   var randomPos = function (halfSize) {
      return $Random.customGenerator(function (seed) {
         return function () {
            var rangeY = A2($Random.$int,
            0 - $Basics.ceiling(halfSize.y),
            $Basics.floor(halfSize.y));
            var rangeX = A2($Random.$int,
            0 - $Basics.ceiling(halfSize.x),
            $Basics.floor(halfSize.x));
            var _ = A2($Random.generate,
            A2($Random.pair,rangeX,rangeY),
            seed);
            var seed$ = function () {
               switch (_.ctor)
               {case "_Tuple2":
   ...
   var randomDir = $Random.customGenerator(function (seed) {
      return function () {
   ...
   var segmentLength = F2(function (pos1,
   pos2) {
      return $Basics.abs(_U.eq(pos1.x,
      pos2.x) ? pos1.y - pos2.y : pos1.x - pos2.x);
   });
   var shortenLine = F2(function (len,
   line) {
      return function () {
         switch (line.ctor)
         {case "::":
            switch (line._1.ctor)
              {case "::": return function () {
                      ...
                      A2($List._op["::"],
                      line._1._0,
                      line._1._1))) : _L.fromArray([line._0
                                                   ,A3(movePosition,
                                                   A2(segmentDir,
                                                   line._0,
                                                   line._1._0),
   ...
   var movePlayer = F2(function (dist,
   player) {
      return function () {
         var _v33 = player.line;
         switch (_v33.ctor)
         {case "::": return function () {
                 var updateLine = shortenLine(player.length);
                 var maybeTurnPos = function () {
                    var _v36 = player.pendingDir;
                    switch (_v36.ctor)
                    {case "Just":
                       return function () {
                            var turnPos = A2(nextTurnPosition,
                            player.dir,
                            _v33._0);
                            return _U.cmp(A2(segmentLength,
                            _v33._0,
                            turnPos),
                            dist) < 1 ? $Maybe.Just(turnPos) : $Maybe.Nothing;
                         }();}
                    return $Maybe.Nothing;
                 }();
                 return function () {
                    switch (maybeTurnPos.ctor)
                    {case "Just":
                       return _U.replace([["line"
                                          ,updateLine(A2($List._op["::"],
                                          maybeTurnPos._0,
                                          A2($List._op["::"],
                                          maybeTurnPos._0,
                                          _v33._1)))]
                                         ,["dir"
                                          ,$Maybe.withDefault(player.dir)(player.pendingDir)]
                                         ,["pendingDir",$Maybe.Nothing]],
                         player);
                       case "Nothing":
                       return _U.replace([["line"
                                          ,updateLine(A2($List._op["::"],
                         ...
                         player);}
                    _U.badCase($moduleName,
                    "between lines 278 and 289");
                 }();
              }();}
         return player;
      }();
   });
   var changePlayerDir = F2(function (dir,
   player) {
      return function () {
         var line = player.line;
         var line$ = $Maybe.withDefault(line)($Maybe.map(function (head) {
            return A2($List._op["::"],
            head,
            line);
         })($List.head(line)));
         return _U.replace([["dir",dir]
                           ,["line",line$]],
         player);
      }();
   });
   var allowedDirChange = F2(function (dir,
   dir$) {
      return _U.eq(dir.x,
      0) && !_U.eq(dir$.x,
      0) ? true : _U.eq(dir.y,
   ...
   var changePlayerPendingDir = F2(function (dir,
   player) {
      return A2(allowedDirChange,
      player.dir,
      dir) ? _U.replace([["pendingDir"
                         ,$Maybe.Just(dir)]],
      player) : player;
   });
   ...
   var randomSeed = function (model) {
      return $Random.initialSeed($Basics.floor($Time.inMilliseconds($Maybe.withDefault(0)(model.clockTime))));
   };
   var makeFood = function (m) {
      return function () {
         var players = $Dict.values(m.players);
         var findEmptyPos = function (seed) {
            return function () {
               var $ = A2($Random.generate,
               randomPos(m.halfSize),
               seed),
               pos = $._0,
               seed$ = $._1;
               return _U.eq(A2(entityAtPos,
               m.foods,
               pos),
               $Maybe.Nothing) && (_U.eq(A2(entityAtPos,
               m.walls,
               pos),
               $Maybe.Nothing) && _U.eq(A2(entityAtPos,
               players,
               pos),
               $Maybe.Nothing)) ? pos : findEmptyPos(seed$);
            }();
         };
         var emptyPos = findEmptyPos(randomSeed(m));
         return {_: {}
                ,line: _L.fromArray([emptyPos
                                    ,emptyPos])};
      }();
   };
   ...
   var Entity = F2(function (a,b) {
      return _U.insert("line",a,b);
   });
   var Live = {ctor: "Live"};
   var makePlayerLive = function (player) {
      return _U.replace([["status"
                         ,Live]],
      player);
   };
   var playerIsLive = function ($) {
   ...
   var Ghost = {ctor: "Ghost"};
   var makePlayer = F2(function (model,
   id) {
      return function () {
         var halfArea = {_: {}
                        ,x: model.halfSize.x / 2
                        ,y: model.halfSize.y / 2};
         var seed = randomSeed(model);
         var _ = A2($Random.generate,
         A2($Random.pair,
         randomPos(halfArea),
         randomDir),
         ...
         return {_: {}
                ...
                ,id: id
                ,length: 3.0
                ,line: _L.fromArray([p2,p1])
                ,pendingDir: $Maybe.Nothing
                ,score: 0
                ,status: Ghost};
      }();
   });
   var killPlayer = F2(function (model,
   player) {
      return function (p) {
         return _U.replace([["score"
                            ,A2(updateScore,
   ...
   _elm.Model.values = {_op: _op
                       ...
                       ,allowedDirChange: allowedDirChange
                       ,changePlayerPendingDir: changePlayerPendingDir
                       ,changePlayerDir: changePlayerDir
...
Elm.Utils.make = function (_elm) {
   ...
   var delayedTask = F2(function (delay,
   task) {
...
Elm.View.make = function (_elm) {
   ...
   var foodSvg = function (_v0) {
      return function () {
         return function () {
            ...
            _L.fromArray([$Svg$Attributes.cx($Basics.toString(x))
                         ,$Svg$Attributes.cy($Basics.toString(y))
                         ,$Svg$Attributes.r("0.4")
                         ,$Svg$Attributes.fill("none")
                         ,$Svg$Attributes.stroke("#FF7061")
                         ,$Svg$Attributes.strokeWidth("0.1")]),
            _L.fromArray([]));
         }();
      }();
   };
   var svgPoint = function (_v12) {
      return function () {
         return A2($Basics._op["++"],
         $Basics.toString(_v12.x),
         A2($Basics._op["++"],
         ",",
         $Basics.toString(_v12.y)));
      }();
   };
   var svgLine = function (points) {
      return function () {
         var seg = F2(function (idx,
         pos) {
            return A2($Basics._op["++"],
            _U.eq(idx,0) ? "M" : "L",
            svgPoint(pos));
         });
         return $String.concat(A2($List.indexedMap,
         seg,
         points));
      }();
   };
   var wallSvg = function (_v14) {
      return function () {
         return A2($Svg.path,
         _L.fromArray([$Svg$Attributes.d(svgLine(_v14.line))
                      ,$Svg$Attributes.stroke("#3E606F")
                      ,$Svg$Attributes.fill("none")
   ...
   var colours = $Array.fromList(_L.fromArray(["#517A48"
                                              ,"#E17D25"
                                              ,"#5F3960"
                                              ,"#469B9E"
                                              ,"#DB4706"]));
   var playerColour = function (idx) {
      return $Maybe.withDefault("")(A2($Array.get,
      A2($Basics._op["%"],
      idx,
      $Array.length(colours)),
      colours));
   };
   var playerSvg = function (_v16) {
      return function () {
         switch (_v16.ctor)
         {case "_Tuple2":
            return A2($Svg.path,
              _L.fromArray([$Svg$Attributes.d(svgLine(_v16._1.line))
                           ,$Svg$Attributes.stroke(playerColour(_v16._0))
                           ,$Svg$Attributes.fill("none")
                           ,$Svg$Attributes.strokeWidth(_U.eq(_v16._1.status,
                           $Model.Ghost) ? "0.75" : "1")
                           ,$Svg$Attributes.strokeLinecap("round")
                           ,$Svg$Attributes.strokeLinejoin("round")
                           ,$Svg$Attributes.opacity(_U.eq(_v16._1.status,
                           $Model.Ghost) ? "0.5" : "1")]),
              _L.fromArray([]));}
         _U.badCase($moduleName,
         "between lines 41 and 50");
      }();
   };
   var scoreSvg = F3(function (x,
   y,
   _v20) {
      return function () {
         return A2($Svg.text$,
         _L.fromArray([$Svg$Attributes.x($Basics.toString(x))
                      ...
                      ,$Svg$Attributes.fontFamily("\"Courier New\", Courier, monospace")
                      ,$Svg$Attributes.fontSize("1")
                      ,$Svg$Attributes.stroke("none")
                      ,$Svg$Attributes.fill(playerColour(_v20.id))]),
         _L.fromArray([$Svg.text(A2($String.padLeft,
         5,
         _U.chr("0"))($Basics.toString(_v20.score)))]));
      }();
   });
   var scoresSvg = F3(function (x,
   y,
   players) {
      return A2($Svg.g,
      _L.fromArray([]),
   ...
   model) {
      return function () {
         var hSize = model.halfSize;
         return A2($Svg.svg,
         _L.fromArray([$Svg$Attributes.style("background-color:#3E606F")
                      ,$Svg$Attributes.width("100%")
                      ,$Svg$Attributes.height("100%")
                      ,$Svg$Attributes.viewBox(A2($Basics._op["++"],
                      $Basics.toString(0 - hSize.x),
                      A2($Basics._op["++"],
                      " ",
                      A2($Basics._op["++"],
                      $Basics.toString(0 - hSize.y),
                      A2($Basics._op["++"],
                      " ",
                      A2($Basics._op["++"],
                      $Basics.toString(2 * hSize.x),
                      A2($Basics._op["++"],
                      " ",
                      $Basics.toString(2 * hSize.y))))))))]),
         _L.fromArray([A2($Svg.rect,
                      _L.fromArray([$Svg$Attributes.x($Basics.toString(0 - hSize.x))
                                   ,$Svg$Attributes.y($Basics.toString(0 - hSize.y))
                                   ,$Svg$Attributes.width($Basics.toString(2 * hSize.x))
                                   ,$Svg$Attributes.height($Basics.toString(2 * hSize.y))
                                   ,$Svg$Attributes.stroke("none")
